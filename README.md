[![Build Status](https://travis-ci.org/repotag/rubydeps.svg?branch=master)](https://travis-ci.org/repotag/rubydeps)
...
* The Moneta store used by the Worker is provided by the Sinatra app Class: `::CuttingEdge.store`.
  * So we must do e.g. `CuttingEdge.set(:store, Moneta.new(:Memory))` to set the `#store` method on the `CuttingEdge` Class.
